...
class DrawScore(ScoreBaseImage):
    ...
    _diff_pg_bg = [
        Image.open(pic_dir / "border_progress_basic.png"),
        Image.open(pic_dir / "border_progress_advanced.png"),
        Image.open(pic_dir / "border_progress_expert.png"),
        Image.open(pic_dir / "border_progress_master.png"),
        Image.open(pic_dir / "border_progress_remaster.png")
    ]
    ...
    def __init__(self, service: ServiceName, image: Image.Image) -> None:
        ...
        self._design_bg = Image.open(self._pic_theme_path / "design.png")
        self._title_lengthen_bg = Image.open(self._pic_theme_path / "title_lengthen.png")
        self._design_text = (
    ...
    def while_rise_pic(self, data: list[RiseResult], low_score: int, isdx: bool):
        ...
        for index, _d in enumerate(data):
            ...
            rate = Image.open(self._pic_theme_path / f"UI_TTR_Rank_{_d.rate}.png").resize((63, 28))
            ...
            self._tb.draw(x + 210, y + 71, 25, f"{_d.old_achievements:.4f}%", diff_color, anchor="mm")
            self._tb.draw(x + 245, y + 96, 17, f"Ra: {_d.old_rating}", diff_color, anchor="mm")
    ...
    def draw_rise(
        ...
        self._sy.draw(700, height - 76, 18, self._design_text, self._default_text_color, "mm")
    ...
    def draw_plan(
        ...
        self._sy.draw(
            700, 77, 25, f"已完成谱面「{len(completed)}」个", 
            self._default_text_color, "mm"
        )
        self._sy.draw(
            1300, 77, 20, f"可使用「/等级列表 {plan}已完成进度」\n查询详细列表", 
            self._default_text_color, "rm", 2, (255, 255, 255, 255), True
        )
        self._sy.draw(
            700, 77 + completed_y, 25, f"未完成谱面「{len(unfinished)}」个", 
            self._default_text_color, "mm"
        )
        self._sy.draw(
            1300, 77, 20, f"可使用「/等级列表 {plan}未完成进度」\n查询详细列表", 
            self._default_text_color, "rm", 2, (255, 255, 255, 255), True
        )
        self._sy.draw(
            700, 77 + completed_y + unfinished_y, 25, f"未游玩谱面「{len(notstarted)}」个", 
            self._default_text_color, "mm"
        )
        ...
        pagemsg = (
            f"共计「{max}」个谱面，剩余「{len(unfinished + notstarted)}」"
            f"个谱面未完成「{plan.upper()}」"
        )
        self._sy.draw(700, self._im.size[1] - 90, 22, pagemsg, self._default_text_color, "mm")
        self._sy.draw(
            700, self._im.size[1] - 30, 25, self._design_text, 
            self._default_text_color, "mm", 2, (255, 255, 255, 255)
        )
        return image_to_bytesio(self._im)
    ...
    def draw_category(
        ...
        if category in [Category.COMPLETED, Category.UNFINISHED]:
            ...
            self._sy.draw(700, self._im.size[1] - 90, 25, pagemsg, self._default_text_color, "mm")
            self._sy.draw(
                700, self._im.size[1] - 30, 25, self._design_text, 
                self._default_text_color, "mm", 2, (255, 255, 255, 255)
            )
