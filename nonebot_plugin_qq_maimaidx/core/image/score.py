...
class DrawScore(ScoreBaseImage):
    ...
    def while_rise_pic(self, data: list[RiseResult], low_score: int, isdx: bool):
        ...
        for index, _d in enumerate(data):
            ...
            self._tb.draw(x + 210, y + 71, 25, f"{_d.oldachievements:.4f}%", diff_color, anchor="mm")
            self._tb.draw(x + 245, y + 96, 17, f"Ra: {_d.oldrating}", diff_color, anchor="mm")
            self._tb.draw(x + 370, y + 71, 25, f"{_d.achievements:.4f}%", diff_color, anchor="mm")
            self._tb.draw(x + 415, y + 96, 17, f"Ra: {_d.rating}", diff_color, anchor="mm")
            self._tb.draw(x + 315, y + 124, 18, f"ds:{_d.rate}", id_color, anchor="lm")
    ...
    def draw_score_list(
        ...
        for num in range(0, len(current_page_result), 20):
            ...
            base_y = idx * height
            self._im.alpha_composite(self._title_lengthen_bg, (475, base_y + 30))

            no_start = start_offset + num + 1
            no_end = start_offset + num + len(result)
            self._fot.draw(
                700, base_y + 77, 28, f"No.{no_start}- No.{no_end}", 
                self._default_text_color, "mm"
            )
        ...
        self._im.alpha_composite(self._design_bg, (200, canvas_h - 113))
        ...
        self._sy.draw(700, canvas_h - 70, 25, footer_text, self._default_text_color, "mm")
        return image_to_bytesio(self._im)