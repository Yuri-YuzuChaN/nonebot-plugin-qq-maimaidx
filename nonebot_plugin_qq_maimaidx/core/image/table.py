...
from ...constants import ACHIEVEMENT_LIST, COMBO_SP, LEVEL_LIST, RANK_SP, SYNC_D_SP
...
class DrawRatingTable:
    ...
    def draw(self) -> BytesIO:
        ...
        tb.draw(
            ...
            ScoreBaseImage._default_text_color, "mm", 5, (255, 255, 255, 255)
        )
        ...
        for n, key in enumerate(STATISTICS_KEYS[1:]):
            ...
            tb.draw(
                x, y, 30, statistics[key], ScoreBaseImage._default_text_color, 
                "mm", 2, (255, 255, 255, 255)
            )
...
class DrawPlateTable:
    ...
    def _process_plate_table_data(self) -> tuple[int, PlateResultMap]:
        ...
        song_id_to_level = {song.song_id: song.difficulties[3].level for song in song_list}
        
        played_map: PlateResultMap = defaultdict(lambda: defaultdict(lambda: [None] * 4))
    ...
    def _process_plate_table_wu_data(self) -> tuple[int, PlateResultMap]:
        ...
        song_id_to_level = {song.song_id: get_display_level(song) for song in wu_song_list}
    ...
    def draw(self) -> BytesIO:
        ...
        im = Image.open(plate_dir / f"{self.plate_name}.png")
        draw = ImageDraw.Draw(im)
        fot = DrawText(draw, FOTNEWRODIN)
        
        im.alpha_composite(self.progress_bg, (175, 20))
        
        plate_bg = plate_dir / f"{self.version}{'極' if plan == '极' else plan}.png"
            ...
            for idx, (song_id, result) in enumerate(songs_dict.items()):
                ...
                for s_idx in hit_slots:
                    if self.is_wu and len(result) == 5:
                        im.alpha_composite(
                            self.finished_bg[s_idx].resize((14, 14)), (x + 1 + 16 * s_idx, y + 64)
                        )
                    else:
                        im.alpha_composite(self.finished_bg[s_idx], (x + 4 + 19 * s_idx, y + 63))
        ...
        fot.draw(
            700, 240, 30, text, ScoreBaseImage._default_text_color, 
            "mm", 3, (255, 255, 255, 255)
        )
        fot.draw(
            1190, 240, 30, f"{round(progress * 100, 2)}%",
            ScoreBaseImage._default_text_color, "rm", 3, (255, 255, 255, 255)
        )
        ...
        for _l in range(len(lv)):
            ...
            fot.draw(
                x, stats_start_y, 40, complete_sum_group, ScoreBaseImage._id_text_color[_l], 
                "mm", 4, (255, 255, 255, 255)
            )
            fot.draw(
                x + _progress_text_x, stats_start_y + 20, 14, f"/{plate_total_count}", 
                ScoreBaseImage._id_text_color[_l], "rd", 3, (255, 255, 255, 255)
            )
            fot.draw(
                x + _progress_text_x, 343, 20, f"{round(progress_group * 100, 2)}%", 
                ScoreBaseImage._default_text_color, "rm", 2, (255, 255, 255, 255)
            )
