...
from ...config import maiconfig, pic_dir
from ..domain.models.score import *
from ..domain.models.song import SimpleSong, Song
...
class DrawScore(ScoreBaseImage):
    ...
    def draw_plan(
        self,
        completed: list[PlayResult],
        completed_y: int,
        unfinished: list[PlayResult],
        unfinished_y: int,
    ...
    def draw_category(
        self, 
        CATEGORY: str, 
        data: list[PlayResult] | list[SimpleSong],
        page: int = 1, 
        end_page: int = 1
    ) -> Image.Image:
        """
    ...
    def draw_scorelist(
        ...
        for n in range(r):
            y = (109 * 4 + 140) * n
            self._im.alpha_composite(self._title_lengthen_bg, (475, 30 + y))
            start = (20 * n + 1) + 80 * (page - 1)
            self._sy.draw(
                700, 77 + y, 28, f"No.{start}- No.{start + len(newdata[n * 20: (n + 1) * 20]) - 1}", 
                self._default_text_color, "mm"
            )
