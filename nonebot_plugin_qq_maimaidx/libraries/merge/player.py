...
from .models.score import PlayResult


def lxns_play_list(score: list[Score]) -> list[PlayResult]:
    return [
        PlayResult(
...
def df_total_play_list(chart: list[ChartInfo]) -> tuple[int, list[PlayResult]]:
    total = 0
    play_list = []
    for v in chart:
        total += v.ra
