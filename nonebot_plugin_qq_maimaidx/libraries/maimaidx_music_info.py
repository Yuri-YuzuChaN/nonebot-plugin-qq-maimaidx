...
from ..config import *
from .image import draw_gradient, image_to_bytesio
...
async def music_play_data_dev(qqid: int, songs: str) -> Union[str, MessageSegment]:
    """
    带Token的谱面游玩
    """
    try:
...
async def update_rating_table() -> str:
    ...
    try:
        ...
        for ra in levelList[5:]:
            ...
            meiryo.draw(750, height - 30, 25, f'Designed by Yuri-YuzuChaN & BlueDeer233 | Generated by {maiconfig.botName} BOT', (103, 20, 141, 255), 'mm', 3, (255, 255, 255, 255))
            y = 120
            for lv in lvlist:
                y += 10
...
async def rating_table_draw(qqid: int, rating: str) -> MessageSegment:
    try:
        version = list(set(_v for _v in plate_to_version.values()))
        data = await maiApi.query_user('plate', qqid=qqid, version=version)
