...
class Draw:
    ...
    async def whiledraw(self, data: Union[List[ChartInfo], List[PlayInfoDefault], List[PlayInfoDev]], best: bool, height: int = 0) -> Image.Image:
        # y为第一排纵向坐标，dy为各排间距
        dy = 170
    ...
    async def draw(self) -> Image.Image:
        ...
        if self.qqId or self.icon:
            try:
                ...
                if self.icon:
                    qqLogo = Image.open(BytesIO(await maiApi.icon(self.icon)))
...
async def generate(qqid: Optional[int] = None, username: Optional[str] = None, icon: Optional[str] = None) -> Union[str, Image.Image]:
    try:
        if username:
            qqid = None
        ...
        draw_best = DrawBest(mai_info, qqid, icon)
