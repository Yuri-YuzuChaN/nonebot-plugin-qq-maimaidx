...
from ..config import *
from .image import image_to_bytesio, image_to_save, text_to_image
...
async def rise_score_data(qqid: int, username: Optional[str], rating: str, score: str) -> Union[str, MessageSegment]:
    """
    上分数据
    ...
    try:
        ...
        msg = MessageSegment.image(await image_to_save(text_to_image(result.strip())))
...
async def player_plate_data(qqid: int, username: Optional[str], ver: str, plan: str, nickname: Optional[str] = '') -> Union[str, MessageSegment]:
    """
    查看将牌
        ...
        if len(song_remain_difficult) > 0:
            if len(song_remain_difficult) < 60:
                ...
                if len(song_remain_difficult) > 10:
                    msg = MessageSegment.image(await image_to_save(text_to_image(msg.strip())))
            else:
...
async def level_achievement_list_data(qqid: int, username: Optional[str], rating: str, page: Optional[str], nickname: Optional[str] = '') -> Union[str, MessageSegment]:
    """
    查看分数列表
    ...
    try:
        ...
        data = f'{appellation}的{rating}分数列表（从高至低）：\n'
        for i, s in enumerate(sorted(song_list, key=lambda i: i['achievements'], reverse=True)):
            if (page - 1) * SONGS_PER_PAGE <= i < page * SONGS_PER_PAGE:
                m = mai.total_list.by_id(str(s['id']))
                data += f'No.{i + 1} {s["achievements"]:.4f} {m.id}. {m.title} {diffs[s["level_index"]]} {m.ds[s["level_index"]]}'
                if s["fc"]: data += f' {comboRank[combo_rank.index(s["fc"])].upper()}'
                if s["fs"]: data += f' {syncRank[sync_rank.index(s["fs"])].upper()}'
                data += '\n'
        data += f'第{page}页，共{len(song_list) // SONGS_PER_PAGE + 1}页'
...
async def rating_ranking_data(name: Optional[str], page: Optional[int]) -> Union[str, MessageSegment]:
    """
    查看查分器排行榜
    ...
    try:
        ...
        if name:
            ...
            data = MessageSegment.image(await image_to_save(text_to_image(msg.strip())))
    except Exception as e:
        log.error(traceback.format_exc())
        data = f'未知错误：{type(e)}\n请联系Bot管理员'
    return data