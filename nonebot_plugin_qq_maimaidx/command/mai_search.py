...
from ..libraries.search_df import draw_df_chart_info
...
async def _(
    ...
    if len(result) == 1:
        await search_music.finish(await draw_df_chart_info(result[0], qqid=user_id))
...
async def _(
    ...
    if alias_data:
        if len(alias_data) != 1:
            ...
            if song:
                msg = "您要找的是不是：" + await draw_df_chart_info(song, qqid=user_id)
            else:
                msg = error_msg
    ...
    if name.isdigit() and (song := mai.total_list.by_id(name)):
        await search_alias_song.finish(
            "您要找的是不是：" + await draw_df_chart_info(song, qqid=user_id)
        )
    ...
    if len(result) == 0:
        ...
        await search_alias_song.finish(
            "您要找的是不是：" + await draw_df_chart_info(result[0], user_id)
        )
    elif len(result) < 50:
...
async def _(
    ...
    if not song:
        ...
        msg = await draw_df_chart_info(song, qqid=user_id)
